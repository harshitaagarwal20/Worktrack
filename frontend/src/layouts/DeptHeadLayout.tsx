...
import {
  LayoutDashboard, Users, CalendarX, Car, FileText, Train,
...
const NAV = [
  ...
  { to: '/leaves',       label: 'Leave Requests',    icon: CalendarX       },
  { to: '/travel',       label: 'Travel Claims',     icon: Car             },
  { to: '/outstation',   label: 'Outstation Claims', icon: Train           },
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex items-center gap-3 px-4 py-4 border-b border-gray-100">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-primary font-semibold tracking-wide">Department Head</p>
        </div>
        {onClose && (
          <button
            onClick={onClose}
        ...
        <div className="flex items-center gap-3 px-2 py-2.5 rounded-xl bg-gray-50">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs font-semibold text-gray-800 truncate">
              {user?.employee ? `${user.employee.firstName} ${user.employee.lastName}` : 'Department Head'}
            </p>
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 min-w-0 lg:ml-60 flex flex-col min-h-screen">
        ...
        <main className="flex-1 min-w-0 overflow-x-hidden p-4 sm:p-5 lg:p-6 animate-fade-in">{children}</main>
      </div>
    </div>
  );
}
