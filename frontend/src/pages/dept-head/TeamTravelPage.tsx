...
import { AlertCircle, Car, Check, Clock, MapPin, X } from 'lucide-react';
...
function initials(item: TravelReimbursement) {
...
export default function TeamTravelPage() {
  ...
  const [selected, setSelected]       = useState<TravelReimbursement | null>(null);
  ...
  function load() {
    setLoading(true);
    travelApi
      .getAllReimbursements({ limit: 200 })
  ...
  function openReview(item: TravelReimbursement, act: 'approve' | 'reject') {
  ...
  async function handleReview() {
    ...
    try {
      if (action === 'approve') {
        await travelApi.approveReimbursement(selected.id, remark.trim() || undefined);
      } else {
        await travelApi.rejectReimbursement(selected.id, remark.trim());
      }
  ...
  return (
    <div className="animate-slide-up">
      <PageHeader
        title="Team Travel Claims"
        subtitle="Review and approve travel expense claims for your team"
      />
      ...
      <Modal
        ...
        subtitle={
          selected
            ? `${selected.employee?.firstName ?? ''} ${selected.employee?.lastName ?? ''} — ₹${fmt(selected.amount)}`
            : ''
        }
          ...
          <div className="space-y-4">
            <div className="bg-gray-50 rounded-xl px-4 py-3 space-y-1.5 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-800">{format(new Date(selected.travelDate), 'd MMM yyyy')}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-800">{selected.kilometers} km × ₹{selected.ratePerKm}/km</span>
              </div>
